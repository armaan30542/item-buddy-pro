...
export default function Return() {
  ...
  const loadLoans = async () => {
    ...
    setLoading(true);
    try {
      setLoans(await getActiveStudentLoans(student.id));
    } catch {
    ...
    <div className="min-h-screen bg-background p-6">
      <div className="mx-auto max-w-3xl animate-fade-in">
        ...
        {loading ? (
          ...
          <div className="rounded-xl border bg-card p-12 text-center"><p className="text-lg text-muted-foreground">No items to return</p></div>
        ) : (
          <div className="grid gap-3">
            {loans.map((loan) => (
              <div key={loan.id} className={`flex items-center justify-between rounded-xl border bg-card p-4 shadow-sm ${isOverdue(loan.due_date) ? "border-destructive/50" : ""}`}>
                <div className="flex-1">
                  ...
                  <div className="mt-1 flex items-center gap-2 text-sm">
                    <span className="text-muted-foreground">Due: {format(new Date(loan.due_date), "MMM d, yyyy")}</span>
                    {isOverdue(loan.due_date) && <span className="status-overdue rounded px-2 py-0.5 text-xs">OVERDUE</span>}
                  </div>
                ...
                <Button onClick={() => handleReturn(loan.id)} disabled={returning === loan.id} variant="outline" className="touch-target">
                  {returning === loan.id ? <Loader2 className="h-4 w-4 animate-spin" /> : <><RotateCcw className="mr-1 h-4 w-4" />Return</>}
                </Button>
              </div>
            ))}
          </div>
